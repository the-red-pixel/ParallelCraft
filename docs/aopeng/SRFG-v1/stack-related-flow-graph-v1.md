...
- [2 图结构](#2&nbsp;图结构)
    - [2.1 基本表示](#2.1&nbsp;基本表示)
    - [2.2 有序性问题](#2.2&nbsp;有序性问题)
    - [2.3 非栈上联结逻辑](#2.3&nbsp;非栈上联结逻辑)
    - [2.4 跳转](#2.4&nbsp;跳转)
- [3 图算法](#3&nbsp;图算法)
    - [3.1 构建](#3.1&nbsp;构建)
        - [3.1.1 构建简单代码的SRFG](#3.1.1&nbsp;构建简单代码的SRFG)
        - [3.1.2 构建存在非栈上联结逻辑的代码的SRFG](#3.1.2&nbsp;构建存在非栈上联结逻辑的代码的SRFG)
        - [3.1.3 构建存在跳转的代码的SRFG](#3.1.3&nbsp;构建存在跳转的代码的SRFG)
    - [3.2 遍历与还原](#3.2&nbsp;遍历与还原)
...
### 2.1&nbsp;基本表示
&emsp;&emsp;在 **SRFG** 中，最基本的单位是**节点**，而节点与节点之间使用**边**进行连结。每条边能且仅能连接两个节点，而每个节点可与多条边联结，与一般的图算
法相类似。  
...
Graph:
    ...
    +————————————————————+
```
&emsp;&emsp;第二步中，遍历到了 ```#2 ireturn``` 指令，该指令有一个栈上操作数，而当前栈信息表中存在一个栈元素，并且该元素来自于 ```#1 iconst_0``` 。所以此时就将 ```#2 ireturn``` 指令直接连接在 ```#1 iconst_0``` 之后，并且栈上来自 ```#1 iconst_0``` 的元素就是 ```#2 ireturn``` 指令的第一操作数。此时，代码段 3-1 已经遍历完毕，整个控制流已经结束。在本节中暂不讨论对于**SRFG**的后期修改，所以块 B0 不再存在**下级块**。至此，整个关于代码段 3-1 的**SRFG**就已经构建完毕了。以上步骤同样可以推广到更复杂的代码的**SRFG**F的构建。更多样例详见 examples 目录。

#### 3.1.2&nbsp;构建存在非栈上联结逻辑的代码的SRFG
...
Stack:
 ...
 AFTER: | I: #1 |       |
        ...
        +———————+-------+
```
&emsp;&emsp;第五步中，```#5 putstatic``` 拥有一个操作数，连接入图时就将栈顶一个元素所指向的节点作为它的**上节点**。而在 ```#5 putstatic``` 连接入图时，栈内已经没有来自**栈相关流** ```SRF 1``` 的元素了，此时也就确定了**待定栈相关流** ```SRF 1``` 和其他的**栈相关流**没有连通性。所以**待定栈相关流** ```SRF 1``` 就可以不再有**待定**的标签，而被正式作为一条**栈相关流**。
```
[STEP 6]
