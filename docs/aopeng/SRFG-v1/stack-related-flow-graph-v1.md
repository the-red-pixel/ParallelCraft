...
#### <a id = "3-2-4">3.2.4&nbsp;包含异常处理的&nbsp;SRFG&nbsp;的遍历与还原</a>
&emsp;&emsp;对于一个包含异常处理的 **SRFG** 的遍历与还原，将以图 3-6 所表示的 **SRFG** 为例，其遍历及还原的过程如下：
```
Graph 3-6:
    +—B0———————————————————————+    +—B1———————————————————+
    |  #SRF Root [0]           |    |  #SRF Root [1]       |
...
Bytecode attribute:
    B0 handled by B2 : java.io.IOException

Transaction table:
    #  BLOCK VISIT [B0] -> [B1]
    #  BLOCK VISIT [B0] -> [B2]
``` 
&emsp;&emsp;对于**块** ```B0``` 内的节点的遍历过程详见前文，在此就不再多作赘述。由于**块** ```B0``` 存在异常处理逻辑的相关信息，所以需要在该**块**中的第一个节点之前加入一个标签，此处为标签 ```L1```。在完成了**块** ```B0``` 内的节点的遍历及还原过程之后，根据**块** ```B0``` 的下级块信息及异常处理属性，在最后一个节点之后加入一个标签，此处为标签 ```L2```，而后将字节码段属性中的相应的异常处理属性完善，将其中的**块** ```B0``` 替换为标签 ```L1``` 到标签 ```L2``` 表示的区间。最后将对于异常处理逻辑**块** ```B2``` 的块访问事务和对下级**块** ```B1``` 的块访问事务依次推入事务表中。
```
Graph 3-6:
    +—B0———————————————————————+    +—B1———————————————————+
    |  #SRF Root [0]           |    |  #SRF Root [1]       |
...
Bytecode attribute:
    L1 to L2 handled by B2 : java.io.IOException

Transaction table:
    #  BLOCK VISIT [B0] -> [B2]
```
&emsp;&emsp;在以上过程中，完成对于**块** ```B1``` 的遍历及还原过程之后，将事务表中所有指向**块** ```B1``` 的块访问事务移除。再根据事务表，继续遍历及还原**块** ```B2```。
```
Graph 3-6:
    +—B0———————————————————————+    +—B1———————————————————+
    |  #SRF Root [0]           |    |  #SRF Root [1]       |
...
Transaction table:
    EMPTY
```
&emsp;&emsp;在以上过程中，首先在**块** ```B2``` 中的第一个节点之前加入一个标签，此处为标签 ```L3```，并将字节码属性中的相应异常处理属性 完善，将**块** ```B2``` 替换为标签 ```L3```，表示异常处理块的起始位置。在完成对**块** ```B2``` 中的所有节点的遍历及还原之后，将事务表中所有指向**块** ```B2``` 的块访问事务移除。此时事务表已为空，对于此 **SRFG** 的遍历及还原的过程就已经全部结束了。以上过程同样可以推广至更复杂的 **SRFG**，更多样例详见 examples 目录。  


## <a id = "4">4&nbsp;例外</a>
&emsp;&emsp;本节将例举出一些违反本版本的 **SRFG** 的准则的错误样例，这些样例所指出的情况一般不会由合法的字节码代码段构建而成，但在人为构建 **SRFG** 的过程中是需要着重注意的，否则可能会得到意料之外的结果。并且此种例外在还原过程中，出于性能因素的考虑，一般不要求进行严格检查。  

### <a id = "4-1">4.1&nbsp;存在连通的栈相关流</a>
```
Graph 4-1:
    #SRF Root [0]       #SRF Root [1]  
    +-------------+     +-------------+
    | #1 iconst_0 |     | #3 iconst_1 |
    +-------------+     +-------------+
         |                  |
    +---------------+       |
    | #2 ESCAPE [1] |       |
...
Bytecode 4-1:
    ...
    invokestatic (II)V
```
&emsp;&emsp;对于图 4-1 所表示的 **SRFG**，其还原过程可能会得到看似合理的结果，如代码段 4-1 所示。而由于**逃逸节点**及**栈相关流**在图中的存在并不符合规则，第 0 **栈相关流**与第 1 **栈相关流**存在着直接连接，而 ```#2 ESCAPE [1]``` 也指向了按照规则不应该存在的第 1 **栈相关流**，故实际上此 **SRFG** 的含义是不明确的。  
&emsp;&emsp;以下样例则能更明显的突出问题：
```
Graph 4-2:
    #SRF Root [0]       #SRF Root [1]  
    +-------------+     +-------------+
...
Bytecode 4-2:
    ...
    invokestatic (II)V
```
&emsp;&emsp;而对于图 4-2 所表示的 **SRFG** 则不能在还原过程中产生合理的结果。按照一般还原过程，会产生如代码段 4-2 所示结果，按照图 4-2 的表示。```#3 iconst_1``` 产生的栈上元素应该在栈顶的位置，而 ```#1 iconst_0``` 产生的栈上元素应该在栈深度为 1 的位置，但是生成的结果所隐含的栈上有序性已经与此相反。  
&emsp;&emsp;由于可能存在的还原过程的不同具体实现，图 4-2 还可能被还原成其它无法预知的结果，所以图 4-2 表示的 **SRFG** 是不合法的，这也是各个**栈相关流**直接严格不允许存在直接连接的原因之一。如果需要不改变某一 **SRFG** 中所有的逻辑顺序，而交换某处栈顶两个或多个元素的顺序，在本修订版 **SRFG** 中，应考虑使用 ```swap``` 指令，如 [4.2](#4.2) 节所示。

### <a id = "4-2">4.2&nbsp;逻辑顺序相关性</a>
```
Graph 4-3:
    #SRF Root [0]            
    +----------------------+
...
Bytecode 4-3:
    ...
    #3 invokestatic f(II)V
```
&emsp;&emsp;对于图 4-3 所表示的 **SRFG**，若我们需要调整 ```#3 invokestatic f(II)V``` 的栈上元素操作数的顺序及其来源上路径的逻辑顺序，只需要对其**上路径**的顺序标号进行调整即可。而在只需要跳转栈上元素操作数的顺序而不需要改变其来源上路径的逻辑顺序时，就应该考虑使用 ```swap``` 指令或其它方法，在调整两个以上数量的栈上元素的顺序时就可能需要借助本地变量操作。
```
Graph 4-4:
    #SRF Root [0]            
    +----------------------+
...
Bytecode 4-4:
    ...
    #3 invokestatic f(II)V
```
&emsp;&emsp;在如图 4-4 所示的仅改变 ```#3 invokestatic f(II)V``` 的**上路径**的顺序标号时，还原时会生成如代码段 4-4 所示的结果，此时 ```#1 invokestatic a()I``` 与 ```#2 invokestatic b()I``` 的逻辑顺序已经发生了改变，对于更加复杂的 **SRFG** 同理。