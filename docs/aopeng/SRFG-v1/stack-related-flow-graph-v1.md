...
Transaction table:
    EMPTY
```
&emsp;&emsp;在以上过程中，首先在**块** ```B2``` 中的第一个节点之前加入一个标签，此处为标签 ```L3```，并将字节码属性中的相应异常处理属性完善，将**块** ```B2``` 替换为标签 ```L3```，表示异常处理块的起始位置。在完成对**块** ```B2``` 中的所有节点的遍历及还原之后，将事务表中所有指向**块** ```B2``` 的块访问事务移除。此时事务表已为空，对于此 **SRFG** 的遍历及还原的过程就已经全部结束了。以上过程同样可以推广至更复杂的 **SRFG**，更多样例详见 examples 目录。  